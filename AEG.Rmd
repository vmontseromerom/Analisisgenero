...
Esta sección sera interesante cerrarla analizando las tasas de participación económica para hombre y mujeres, como nos indica su nombre, mide el grado de participación de la población en la economía.
...
```{r}
kbl(seleccion_ENOE_13 %>% filter(eda>14 & eda<=75) %>% mutate_at(vars(sex, clase1), as_label) %>% tabyl(edaq,clase1, sex, show_missing_levels = F) %>% adorn_percentages("row") %>% adorn_pct_formatting()) %>% kable_classic()
...
```{r}
kbl(seleccion_ENOE_18 %>% filter(eda>14 & eda<=75) %>% mutate_at(vars(sex, clase1), as_label) %>% tabyl(edaq,clase1, sex, show_missing_levels = F) %>% adorn_percentages("row") %>% adorn_pct_formatting()) %>% kable_classic()
...
```{r}
g_tasa0+geom_line(aes(color=sex)) + ggtitle("Tasas de participació econmica por grupo de edad, trimestre I, 2013")+theme(axis.text.x = element_text(angle = 90))
